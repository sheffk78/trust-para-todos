/**
 * Trust Para Todos — i18n
 *
 * Simple dictionary-based translation for admin and customer panels.
 * Language is stored in a cookie: tpt_lang = 'es' | 'en'
 */
...
export function getPlanLabel(plan: string, locale: Locale): string {
  return t(`plan.${plan}`, locale) || plan;
}

export function getStepLabel(name: string, locale: Locale): string {
  return t(`step.${name}`, locale) || name;
}

export function getStepDescription(name: string, locale: Locale): string {
  return t(`step.desc.${name}`, locale) || '';
}

export function getDocLabel(type: string, locale: Locale): string {
  return t(`doc.${type}`, locale) || type;
}

export function getDocStatusLabel(status: string, locale: Locale): string {
  return t(`doc.${status}`, locale) || status;
}

export function getLocaleFromCookie(cookieHeader: string | null): Locale {
  if (!cookieHeader) return 'es';
  const match = cookieHeader.match(/tpt_lang=([^;]+)/);
  return getLocale(match?.[1]);
}
